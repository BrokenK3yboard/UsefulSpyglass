...
public class RayTracingMixin {
    ...
    private HitResult modifyRange(RayTracing instance, Entity lookVector, double entityDist, double blockDist, Operation<HitResult> original, @Local Player player) {
        return ClientConfig.JADE_INTEGRATION.get() && player.getUseItem().getItem() instanceof SpyglassItem ?
                original.call(instance, lookVector, 100D, 100D) : original.call(instance, lookVector, entityDist, blockDist);
    }
}
