...
public class InfoOverlay {
    ...
    public static void setHitResult(HitResult result) {
        ...
        if(hitResult instanceof EntityHitResult entityHit && entityHit.getEntity() instanceof LivingEntity entity) {
            ...
            array.add(new MobInfo(Icon.HEALTH, ClientTooltipComponent.create(new TextComponent("x " + (int)entity.getHealth()).getVisualOrderText())));

            if (entity.getArmorValue() > 0)
                array.add(new MobInfo(Icon.ARMOR, ClientTooltipComponent.create(new TextComponent("x " + entity.getArmorValue()).getVisualOrderText())));
            ...
            if(arr.length > 1) {
                ...
                for (String addStr : arr) {
                    if (ClientTooltipComponent.create(new TextComponent(str + " " + addStr).getVisualOrderText()).getWidth(CLIENT.font) > maxLength && str.length() > 0) {
                        array.add(new BlockInfo(stack, ClientTooltipComponent.create(new TextComponent(str.toString()).getVisualOrderText())));
                ...
                array.add(new BlockInfo(stack, ClientTooltipComponent.create(new TextComponent(displayStr).getVisualOrderText())));
            }
            setComponent(array);
            hitResult = result;
        }
