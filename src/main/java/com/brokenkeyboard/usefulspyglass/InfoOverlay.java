    ...
    public static void setHitResult(HitResult result) {
        ...
        if(hitResult instanceof EntityHitResult entityHit && entityHit.getEntity() instanceof LivingEntity entity) {
            ...
            if (entity instanceof NeutralMob || entity instanceof Player) {
                mobRelation = new TranslatableComponent("text." + UsefulSpyglass.MOD_ID + ".neutral").withStyle(ChatFormatting.YELLOW);
            } else if (entity.getType().getCategory().isFriendly()) {
                mobRelation = new TranslatableComponent("text." + UsefulSpyglass.MOD_ID + ".friendly").withStyle(ChatFormatting.GREEN);
            } else {
                mobRelation = new TranslatableComponent("text." + UsefulSpyglass.MOD_ID + ".hostile").withStyle(ChatFormatting.RED);
            }
            ...
            array.add(new TooltipInfo.MobInfo(TooltipInfo.Icon.HEALTH, ClientTooltipComponent.create(new TextComponent("x " + (int)entity.getHealth()).getVisualOrderText())));

            if (entity.getArmorValue() > 0)
                array.add(new TooltipInfo.MobInfo(TooltipInfo.Icon.ARMOR, ClientTooltipComponent.create(new TextComponent("x " + entity.getArmorValue()).getVisualOrderText())));
            ...
            if (ClientConfig.DISPLAY_ENTITY_NAMEPSPACE.get()) {
                ...
                if (location != null && getModName(location.getNamespace()) != null) {
                    array.add(new TooltipInfo.MobInfo(TooltipInfo.Icon.NONE, ClientTooltipComponent.create(new TextComponent(getModName(location.getNamespace())).
            ...
            if(arr.length > 1) {
                ...
                for (String addStr : arr) {
                    if (ClientTooltipComponent.create(new TextComponent(str + " " + addStr).getVisualOrderText()).getWidth(CLIENT.font) > maxLength && str.length() > 0) {
                        array.add(new TooltipInfo.BlockInfo(stack, ClientTooltipComponent.create(new TextComponent(str.toString()).getVisualOrderText())));
                ...
                array.add(new TooltipInfo.BlockInfo(stack, ClientTooltipComponent.create(new TextComponent(displayStr).getVisualOrderText())));
            }

            if (ClientConfig.DISPLAY_BLOCK_NAMEPSPACE.get()) {
                ResourceLocation location = ForgeRegistries.BLOCKS.getKey(state.getBlock());
                if (location != null && getModName(location.getNamespace()) != null) {
                    array.add(new TooltipInfo.BlockInfo(stack, ClientTooltipComponent.create(new TextComponent(getModName(location.getNamespace())).
                            withStyle(ChatFormatting.BLUE).getVisualOrderText())));
                }
            }
